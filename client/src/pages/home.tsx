    ...
    <div className="w-full h-full relative overflow-hidden">
      {isVideo ? (
        <video
          ...
          loop
          playsInline
          poster={slide.imageUrl ? toWebP(slide.imageUrl, 800) : undefined}
        />
      ) : slide.imageUrl ? (
        <img
          src={toWebP(slide.imageUrl, priority ? 800 : 1200)}
          srcSet={`${toWebP(slide.imageUrl, 400)} 400w, ${toWebP(slide.imageUrl, 800)} 800w, ${toWebP(slide.imageUrl, 1200)} 1200w`}
...
export default function Home() {
  ...
  const { data: bannerSlides = [] } = useQuery<BannerSlideData[]>({
    ...
    // Cache el banner por 5 minutos — no necesita refrescarse en cada visita
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <AppLayout>
      ...
      >
        ...
        >
          {/* Fondo de color instantáneo — visible ANTES de que cargue el banner */}
          <div className="absolute inset-0 bg-gray-100" />

          {hasDynamicSlides && (
            <>
              {/* Primer slide: sin AnimatePresence ni fade — aparece instantáneo */}
              {currentSlide === 0 && (
                <div className="absolute inset-0">
                  <DynamicBannerSlide
                    slide={bannerSlides[0]}
                    priority={true}
                  />
                </div>
              )}

              {/* Slides siguientes: con transición suave */}
              {currentSlide > 0 && (
          ...
          {totalSlides > 1 && hasDynamicSlides && (
            <>
              <button
                onClick={prevSlide}
